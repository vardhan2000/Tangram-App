		...
		switch(key){
			case "(":
				for (let i = 0; i < this.primitives.length; i++) {
                    ...
                    prim.transform.setTranslate([Math.cos(-rotate_by) * (prim.transform.getTranslate()[0] - cx) - Math.sin(-rotate_by) * (prim.transform.getTranslate()[1] - cy) + cx, Math.sin(-rotate_by) * (prim.transform.getTranslate()[0] - cx) + Math.cos(-rotate_by) * (prim.transform.getTranslate()[1] - cy) + cy, 0]);
